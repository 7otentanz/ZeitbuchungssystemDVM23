...
def berichtAnlegen(request):
	...
	if request.method == "POST":
		...
		if not id:
			startzeit = datetime.now(ZoneInfo("Europe/Berlin"))
			try:
				with open(jsonDatei, "r", encoding="utf-8") as datei:
					daten = json.load(datei)
			...
			endzeit = datetime.now(ZoneInfo("Europe/Berlin"))
			endzeitBerichte = 0
			startzeitBerichte = 0
...
def gesamtarbeitszeit(request):
	...
	if request.method == "POST":
		...
		for bericht in bestehendeBerichte:
			endzeitBerichte = endzeitBerichte + bericht["endzeit"]
			startzeitBerichte = startzeitBerichte + bericht["startzeit"]
			gesamtarbeitszeit = (endzeitBerichte - startzeitBerichte).total_seconds() / 60
		
		parameter = loginPruefen(request)
		parameter.update({"Gesamtarbeitszeit" : gesamtarbeitszeit})
		return render(request, 'lassMichDasZusammenfassen.html', parameter)
